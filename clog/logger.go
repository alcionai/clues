...
func PlantLogger[CTX valuer](
	ctx CTX,
	seed *zap.SugaredLogger,
) CTX {
...
func plantLoggerInCtx[CTX valuer](
	ctx CTX,
	clogger *clogger,
) CTX {
	if clogger == nil {
	...
	if cCtx, ok := valuer(ctx).(context.Context); ok {
		return context.WithValue(cCtx, ctxKey, clogger).(CTX)
	}

	// temporal workflow.Context
	if tCtx, ok := valuer(ctx).(workflow.Context); ok {
		return workflow.WithValue(tCtx, ctxKey, clogger).(CTX)
	}
...
func fromCtx(ctx valuer) *clogger {
	l := ctx.Value(ctxKey)
...
func Singleton() *builder[context.Context] {
	if cloggerton == nil {
