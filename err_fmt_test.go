...
var (
	...
	cluErr  = clues.Sentinel("an error")

	cluesWrap       = func(err error) error { return clues.Wrap(nil, err, "clues wrap") }
	cluesPlainStack = func(err error) error { return clues.Stack(nil, err) }
	cluesStack      = func(err error) error { return clues.Stack(nil, globalSentinel, err) }
)
...
func TestFmt(t *testing.T) {
	...
	table := []struct {
		...
		// ---------------------------------------------------------------------------
		{
			...
			onion: clues.Stack(nil,
				clues.Wrap(nil, clues.Stack(nil,
					stderr.New("top"),
					stderr.New("mid"),
				), "lhs"),
				clues.Wrap(nil, clues.Stack(nil,
...
func bottomWrap(err error) error {
	return clues.Wrap(nil, err, "bottom wrap")
}

func midWrap(err error) error {
	return clues.Wrap(nil, bottomWrap(err), "mid wrap")
}

func topWrap(err error) error {
	return clues.Wrap(nil, midWrap(err), "top wrap")
}

// plain-stacked

func bottomPlainStack(err error) error {
	return clues.Stack(nil, err)
}

func midPlainStack(err error) error {
	return clues.Stack(nil, bottomPlainStack(err))
}

func topPlainStack(err error) error {
	return clues.Stack(nil, midPlainStack(err))
}

// stacked

func bottomStack(err error) error {
	return clues.Stack(nil, clues.New(nil, "bottom"), err)
}

func midStack(err error) error {
	return clues.Stack(nil, clues.New(nil, "mid"), bottomStack(err))
}

func topStack(err error) error {
	return clues.Stack(nil, clues.New(nil, "top"), midStack(err))
}
...
func TestSkipCaller(t *testing.T) {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("wrap", func(t *testing.T) {
				checkFmt{"%+v", "", regexp.MustCompile(test.expectWrapped)}.
					check(t, clues.Wrap(nil, test.tracer(cluErr), "wrap"))
			})
			t.Run("stack", func(t *testing.T) {
				checkFmt{"%+v", "", regexp.MustCompile(test.expectStacked)}.
					check(t, clues.Stack(nil, cluErr, test.tracer(cluErr)))
			})
		})
	}
	table2 := []struct {
	...
	for _, test := range table3 {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("wrap", func(t *testing.T) {
				checkFmt{"%+v", "", regexp.MustCompile(test.expectWrapped)}.
					check(t, clues.Wrap(nil, test.tracer(err), "wrap"))
			})
			t.Run("stack", func(t *testing.T) {
				checkFmt{"%+v", "", regexp.MustCompile(test.expectStacked)}.
					check(t, clues.Stack(nil, err, test.tracer(err)))
			})
		})
	}
}
...
func TestComment(t *testing.T) {
	...
	for _, test := range table {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("wrap", func(t *testing.T) {
				err := clues.Wrap(nil, test.commenter(cluErr), "wrap")
				comments := clues.Comments(err).String()
				commentMatches(t, test.expectWrapped, comments)
			})
			t.Run("stack", func(t *testing.T) {
				err := clues.Stack(nil, cluErr, test.commenter(cluErr))
				comments := clues.Comments(err).String()
	...
	for _, test := range table2 {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("stack", func(t *testing.T) {
				err := clues.Stack(nil, nil, cluErr, test.commenter(cluErr))
				comments := clues.Comments(err).String()
				commentMatches(t, test.expectStacked, comments)
			})
		})
